...
const DEFAULT_REFLECT = `REFLECTION CHECKPOINT
...
function makeState(overrides: Partial<LoopState> = {}): LoopState {
  return {
    ...
    itemsPerIteration: 0,
    reflectEvery: 0,
    reflectInstructions: DEFAULT_REFLECT,
    active: true,
    status: "active",
...
describe("LoopOrchestrator", () => {
  ...
  describe("start", () => {
    it("creates a new loop state and returns it with a prompt", () => {
      const result = orchestrator.start("my-plan", {
    ...
    it("persists state via the store", () => {
      orchestrator.start("my-plan", {
        taskFile: ".scratch/my-plan/issues/01-foo.md",
        taskContent: "Task content.",
      }, ctx);

      expect(store.saveState).toHaveBeenCalledOnce();
      const saved = (store.saveState as ReturnType<typeof vi.fn>).mock.calls[0][1];
    ...
    it("does not mark as plan-level for issue-level loops", () => {
      const result = orchestrator.start("my-plan/01-foo", {
        taskFile: ".scratch/my-plan/issues/01-foo.md",
        taskContent: "Task.",
      }, ctx);
  ...
  describe("advance", () => {
    ...
    it("includes PRD content in prompt for plan-level loops", () => {
      const state = makeState({ name: "my-plan" });
      const result = orchestrator.advance(state, "Task content", "## PRD\n\nContext.", ctx);

      expect(result.prompt).toContain("Context.");
      expect(result.prompt).toContain("PLAN-LEVEL");
    });

    it("returns prompt even when prompt string would be empty after completion", () => {
  ...
  describe("integration with real LoopStore", () => {
    ...
    it("start creates state file on disk", () => {
      ...
      fs.writeFileSync(path.join(issuesDir, "01-task.md"), "## Task\n\n- [ ] Do something\n", "utf-8");
      fs.writeFileSync(path.join(planDir, "PRD.md"), "# PRD: My Plan\n", "utf-8");
    ...
    it("full lifecycle: start → advance → stop", () => {
      ...
      const issuesDir = path.join(planDir, "issues");
      fs.mkdirSync(issuesDir, { recursive: true });
      fs.writeFileSync(path.join(issuesDir, "03-task.md"), "## Task\n\n- [ ] Step 1\n", "utf-8");
      ...
      // advance
      const adv1 = realOrch.advance(startResult!.state, "## Task\n\n- [x] Step 1\n", realCtx);

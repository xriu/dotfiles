...
function getActiveProfile(): string {
	...
	try {
		const state = JSON.parse(
			readFileSync(profileStatePath, "utf8"),
		) as ProfileState;
...
export default function profileModelFooter(pi: ExtensionAPI) {
			...
			return {
				...
				render(width: number): string[] {
					...
					if (statsLeftWidth > effectiveWidth) {
						statsLeft = truncateToWidth(statsLeft, effectiveWidth, "...");
					...
					if (totalNeeded <= effectiveWidth) {
						const padding = " ".repeat(
							effectiveWidth - statsLeftWidth - rightSideWidth,
						);
						statsLine = statsLeft + padding + rightSide;
					} else {
						const availableForRight =
							effectiveWidth - statsLeftWidth - minPadding;
						if (availableForRight > 0) {
							...
							const padding = " ".repeat(
								Math.max(
									0,
									effectiveWidth - statsLeftWidth - truncatedRightWidth,
								),
							);
	...
	pi.registerCommand("footer-profile", {
		description:
			"Toggle profile-aware footer: /footer-profile [on|off|toggle|status]",
		handler: async (args, ctx) => {
			...
			if (input === "off") {
				...
				applyFooterMode(ctx);
				ctx.ui.notify(
					"Profile footer disabled; using Pi default footer",
					"info",
				);
				return;
			}
			...
			if (input === "toggle") {
				...
				applyFooterMode(ctx);
				ctx.ui.notify(
					settings.customProfileFooter
						? "Profile footer enabled"
						: "Profile footer disabled; using Pi default footer",
					"info",
				);
				return;

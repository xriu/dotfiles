...
export default function (pi: ExtensionAPI) {
  ...
  function resolvePath(rawPath: string, cwd: string) {
    ...
    if (cleaned.startsWith("~/")) return normalize(resolve(home, cleaned.slice(2)));
    if (cleaned.startsWith("$HOME/")) return normalize(resolve(home, cleaned.slice(6)));
  ...
  function matchesRoot(target: string, cwd: string, root: string) {
    const absoluteRoot = root.startsWith("/") ? normalize(root) : normalize(resolve(cwd, root));
    return isWithin(target, absoluteRoot);
  }

  function isSensitivePath(rawPath: string, cwd: string, mode: "read" | "write") {
  ...
  pi.on("tool_call", async (event, ctx) => {
    if ((event.toolName === "read" || event.toolName === "edit" || event.toolName === "write") && typeof event.input.path === "string") {
    ...
    if (event.toolName === "bash" && typeof event.input.command === "string") {
      ...
      if (bashSensitivePatterns.some((pattern) => pattern.test(command))) {
        return {
          block: true,
          reason: "bash blocked because command appears to access a protected secret path",
        };
      }
    }

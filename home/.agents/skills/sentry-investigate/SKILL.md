...
description: Investigate and analyze Sentry crash reports to understand root causes and produce minimal reproduction test cases. Use when asked to analyze Sentry crashes, understand error contexts, or create tests that reproduce observed Sentry issues. Methodically works through stack traces, event data, and source code to identify the sequence of events leading to a crash.
license: Apache-2.0
---
...
If given a Sentry issue ID (numeric ID), there are several ways to fetch the crash data:
...
```
scripts/sentry-fetch.sh <issue-id>
```
